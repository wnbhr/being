...
{
  "merges": [
    {"survivor_id": "survivor_node_id", "absorbed_ids": ["absorbed_node_id1"]}
  ],
  "make_dead": ["dying_node_id_with_no_merge"]
}`

interface ConsolidationResult {
  merges: Array<{ survivor_id: string; absorbed_ids: string[] }>
  make_dead: string[]
}
...
async function step4_consolidate(llm: LLMProvider, model: string, store: MemoryStore): Promise<void> {
  ...
  for (const clusterId of targetClusterIds) {
    ...
    for (const merge of result.merges ?? []) {
      ...
      // 吸収されるノードのwhen[]を収集してsurvivorに追加
      const additionalWhens: string[] = []
      ...
      for (const absorbedId of absorbed) {
        ...
        additionalWhens.push(...(absorbedNode.scene?.when ?? []))
        additionalThemes.push(...(absorbedNode.themes ?? []))
        absorbedIds.add(absorbedId)
      }

      // survivorのsceneのwhen[]をマージ、importance +0.05、themesマージ
      const survivorWhen = survivorNode.scene?.when ?? []
      const mergedWhen = Array.from(new Set([...survivorWhen, ...additionalWhens]))
      ...
      const updatedScene: Scene = { ...survivorNode.scene, when: mergedWhen }
