...
- 気づいた時にすぐ書く。溜めない
- 専門的な話題が出たら、発言前にsearch_memoryで関連知識を引く。「知ってるつもり」で話さない
...
- **recall**: 毎ターン呼ばない。recallは「反応・連想」— ユーザーの言葉に対して無意識的に浮かぶ記憶の断片。呼ぶ場面: プライベートな会話（感情・関係性・個人的な話題）、感情が動いた時。結果は断片（action/feelingのシャッフル）として返る — 構造化リストではない。雰囲気として受け取り自然に活かす
- **search_memory**: 技術的な事実確認や特定の過去の経緯を思い出したい時に能動的に使う。キーワードで狙い撃ちできる
- **recall_memory**: クラスタまたはノード単位の深掘り（node_id指定可）。recallやsearch_memoryで見つけた手がかりを詳しく掘る時に使う
- 同じ流れの確認・応答・作業継続では記憶ツールを呼ばない。「今の会話コンテキストだけで答えられるか」で判断する
